...
// CreateResultsArchive crée une archive des résultats et la sauvegarde
// @Summary Create course results archive
// @Description Creates an archive of course results and stores it for later download
// @Tags Archive
// @Accept json
// @Produce json
// @Param course_id path string true "Course ID"
// @Param request body ArchiveRequest true "Archive creation request"
// @Success 201 {object} map[string]interface{} "Archive created"
// @Failure 400 {object} map[string]interface{} "Validation error"
// @Failure 404 {object} map[string]interface{} "Course not found"
// @Failure 500 {object} map[string]interface{} "Internal error"
// @Router /api/v1/storage/courses/{course_id}/archive [post]
func (h *ArchiveHandlers) CreateResultsArchive(c *gin.Context) {
	...
	// Filtrer les fichiers selon include/exclude
	filteredFiles := h.filterFiles(resultFiles, req.Include, req.Exclude)

	// Créer l'archive temporaire
	archiveData, err := h.createArchiveInMemory(courseID, filteredFiles, req.Format, req.Compress)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
	...
	// Sauvegarder l'archive dans le storage
	timestamp := time.Now().Format("20060102-150405")
	archiveFilename := fmt.Sprintf("archive-%s.%s", timestamp, req.Format)

	if err := h.storageService.UploadResult(c.Request.Context(), courseID, archiveFilename, archiveData); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": "Failed to save archive: " + err.Error(),
		})
		return
	}
	...
	c.JSON(http.StatusCreated, gin.H{
		...
		"archive": gin.H{
			...
			"format":       req.Format,
			"files_count":  len(filteredFiles),
			"compressed":   req.Compress,
			"download_url": fmt.Sprintf("/api/v1/storage/courses/%s/results/%s", courseID, archiveFilename),
		},
	})
}
