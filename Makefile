...
# Lister les thèmes disponibles avec statut d'installation
themes-list:
	@echo "📋 Available Slidev themes with installation status:"
...
themes-install:
	@if [ -z "$(THEME)" ]; then \
		echo "❌ Usage: make themes-install THEME=@slidev/theme-seriph"; \
		exit 1; \
	fi
...
# Détecter les thèmes pour un job
themes-detect:
	@if [ -z "$(JOB_ID)" ]; then \
		echo "❌ Usage: make themes-detect JOB_ID=your-job-id"; \
		exit 1; \
	fi
	@echo "🔍 Detecting themes for job: $(JOB_ID)"
	@curl -s http://localhost:8081/api/v1/themes/jobs/$(JOB_ID)/detect | jq .

# Installer automatiquement les thèmes manquants pour un job
themes-auto-install:
	@if [ -z "$(JOB_ID)" ]; then \
		echo "❌ Usage: make themes-auto-install JOB_ID=your-job-id"; \
		exit 1; \
	fi
	@echo "🚀 Auto-installing missing themes for job: $(JOB_ID)"
	@curl -s -X POST http://localhost:8081/api/v1/themes/jobs/$(JOB_ID)/install | jq .

# Test complet avec thème
test-theme-workflow:
	@echo "🧪 Testing complete theme workflow..."
	@echo "1️⃣ Creating test files with theme..."
	@mkdir -p test-files
	@echo 0
	...
	echo "2️⃣ Uploading test files for job: $$JOB_ID"; \
	...
	echo "3️⃣ Detecting themes for job: $$JOB_ID"; \
	curl -s "http://localhost:8081/api/v1/themes/jobs/$$JOB_ID/detect" | jq .;
	@rm -rf test-files

# Test d'installation des thèmes populaires
themes-install-popular:
	@echo "📦 Installing popular Slidev themes..."
